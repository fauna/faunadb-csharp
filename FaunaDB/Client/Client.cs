...
namespace FaunaDB.Client
{
    ...
    class DefaultClientIO : IClientIO
    {
        ...
        {
            var cred = user == null ? password : string.Format("{0}:{1}", user, password);
    ...
    public class Client
    {
        ...
        /// <param name="client">Optional IInnerClient. Used only for testing.</param>"> 
        public Client(
        ...
        {
            return Execute(HttpMethodKind.Get, path, query: query);
        }
        ...
        {
            return Execute(HttpMethodKind.Post, path, data: data);
        }
        ...
        {
            return Execute(HttpMethodKind.Put, path, data: data);
        }
        ...
        {
            return Execute(HttpMethodKind.Patch, path, data: data);
        }
        ...
        {
            return Execute(HttpMethodKind.Delete, path);
        }
        ...
        /// <exception cref="FaunaException"/>
        public Task<Value> Query(Value expression)
        {
            return Execute(HttpMethodKind.Post, "", data: expression);
        }
        ...
        {
            return (string) await Get("ping", ImmutableUtil.DictWithoutNullValues(
        ...
        async Task<Value> Execute(HttpMethodKind action, string path, Value data = null, IReadOnlyDictionary<string, string> query = null)
        {
            ...
            var responseHttp = await PerformRequest(action, path, data, query);
            Console.WriteLine("!!!");
            Console.WriteLine(responseHttp);
            Console.WriteLine("!!!");
            var responseText = await responseHttp.Content.ReadAsStringAsync();
