...
namespace FaunaDB.Client
{
    ...
    public class FaunaClient
    {
        ...
        public async Task<Value> Query(Expr expression) =>
            await Query<Value>(expression).ConfigureAwait(false);
        ...
        /// </summary>
        /// <param name="expression">Expression generated by methods of <see cref="Language"/>.</param>
        public async Task<T> Query<T>(Expr expression) =>
            await Execute<T>(HttpMethodKind.Post, "", expression).ConfigureAwait(false);
        ...
        public async Task<string> Ping(string scope = null, int? timeout = null) =>
            await Execute<string>(HttpMethodKind.Get, "ping", query: ImmutableDictionary.Of("scope", scope, "timeout", timeout?.ToString())).ConfigureAwait(false);

        async Task<T> Execute<T>(HttpMethodKind action, string path, Expr data = null, IReadOnlyDictionary<string, string> query = null)
        {
            ...
            var responseContent = FromJson<T>(responseHttp.ResponseContent);
        ...
        static Response<T> FromJson<T>(string json)
        {
            try
            {
                ...
                return JsonConvert.DeserializeObject<Response<T>>(json, settings);
            }
